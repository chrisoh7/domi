-- 1. Add tasks table to realtime publication (enables live updates in widget + feed)
do $$ begin
  alter publication supabase_realtime add table public.tasks;
exception when others then null; -- already added
end $$;

-- 2. Fix notification trigger — remove all remaining "poster" references
...
begin
  ...
  -- open → pending_runner_approval (domi applied, requester must approve)
  ...
  -- pending_runner_approval → accepted (requester approved domi)
  elsif old.status = 'pending_runner_approval' and new.status = 'accepted' then
    insert into public.notifications(user_id, task_id, type, title, body)
      values (new.runner_id, new.id, 'task_accepted',
              'You''ve been approved',
              'You''ve been approved as domi for "' || tname || '"');

  -- pending_runner_approval → open (requester rejected domi)
    ...
    if old.runner_id is not null then
      insert into public.notifications(user_id, task_id, type, title, body)
        values (old.runner_id, new.id, 'runner_joined',
                ...
                'Your application for "' || tname || '" was not approved.');
    end if;

...
import { Coins, Zap, MapPin, Navigation, Loader2, UserCheck, ShieldAlert, Plus, X, DollarSign, Package, Handshake, Users } from 'lucide-react'
...
export default function PostTask() {
  ...
  return (
    <div className="max-w-xl mx-auto px-4 py-6">
      <h1 className="text-2xl font-bold text-[#1A1A2E] mb-6">Post a Task</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
              ...
              {EST_OPTIONS.map(opt => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-150 border
                    ${estMinutes === opt.value
                      ? 'bg-[#1A1A2E] text-white border-[#1A1A2E] scale-105'
            ...
            {CATEGORIES.map(cat => {
              ...
              return (
                <button
                  ...
                  className={`
                    flex flex-col items-center gap-1.5 p-3 rounded-xl border-2 cursor-pointer select-none
                    transition-all duration-200 ease-out
                    ${active
                      ? 'border-[#C41230] bg-red-50 shadow-sm scale-[1.04]'
                      : 'border-gray-200 bg-white hover:border-gray-300 hover:bg-gray-50 hover:scale-[1.02]'
                    }
                  `}
                >
                  <span className="text-2xl transition-transform duration-200" style={{ transform: active ? 'scale(1.2)' : 'scale(1)' }}>
                    {cat.emoji}
                  </span>
                  <span className={`text-xs font-semibold text-center leading-tight transition-colors ${active ? 'text-[#C41230]' : 'text-gray-700'}`}>
                    {cat.id}
                  </span>
            ...
            {DELIVERY_TYPES.map(dt => {
              ...
              return (
                <button
                  ...
                  `}
                >
                  <span className={`transition-colors ${active ? 'text-[#C41230]' : 'text-gray-500'}`}>
                    {dt.emoji}
                  </span>
                  <span className={`text-xs font-semibold text-center leading-tight transition-colors ${active ? 'text-[#C41230]' : 'text-gray-700'}`}>
                    {dt.label}
              ...
              <div key={idx} className="flex items-start gap-2">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  <button
                    ...
                    className="p-2.5 bg-[#C41230] text-white rounded-lg hover:bg-[#a00f28] transition-colors disabled:opacity-60"
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-5">
          ...
          <input
            ...
            max={100}
            value={tokenOffer}
            onChange={e => setTokenOffer(Number(e.target.value))}
          ...
          <div className="mt-4 border-t border-gray-100 pt-4">
            <label className="flex items-center gap-3 cursor-pointer" onClick={() => setRequiresApproval(r => !r)}>
              <div className="relative flex-shrink-0">
                <div className={`w-10 h-5 rounded-full transition-colors duration-200 ${requiresApproval ? 'bg-[#1A1A2E]' : 'bg-gray-200'}`} />
                <div className={`absolute top-0.5 left-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform duration-200 ${requiresApproval ? 'translate-x-5' : ''}`} />
              </div>
              <div>
                <div className="flex items-center gap-1.5 text-sm font-semibold text-gray-700">
                  <UserCheck size={14} className="text-[#1A1A2E]" />
                  Require runner approval
                </div>
                <p className="text-xs text-gray-500">You confirm the runner before they start</p>
              </div>
            </label>
          </div>
        </div>
        ...
        <div className="bg-gray-50 rounded-xl p-4 flex items-center justify-between">
          <div>
            ...
            <p className="text-lg font-bold text-[#1A1A2E] flex items-center gap-1">
          ...
          <div className="text-right">
            ...
            <p className={`text-lg font-bold ${isDevMode ? 'text-amber-500' : canAfford ? 'text-green-600' : 'text-red-600'}`}>
        ...
        {!canAfford && !isDevMode && (
          <p className="text-center text-sm text-gray-500">
            <a href="/tokens" className="text-[#C41230] font-medium hover:underline">Buy tokens</a> to post this task.
          </p>
        )}
      </form>
    </div>
